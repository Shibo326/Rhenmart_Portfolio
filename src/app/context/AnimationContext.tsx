...
export interface AnimationConfig {
  ...
  // Mobile animation budget flag — true when device is mobile/iOS and
  // infinite animations must be suppressed to maintain 60fps
...
export function computeAnimationConfig(input: DeviceInput): AnimationConfig {
  ...
  return {
    ...
    enableBackdropBlur,
    // Mobile animation budget: disable infinite loops and shimmer on all mobile/iOS
    // devices to prevent GPU overload and maintain 60fps during scroll.
    // Previously: tier !== 'low' (allowed mid-tier Android mobile to run infinite loops)
    // Fixed:      tier !== 'low' && !isMobile (mobile never runs infinite loops)
    enableInfiniteLoops: tier !== 'low' && !isMobile,
    enableShimmer: tier !== 'low' && !isIOS && !isMobile,
    enable3DScene,
    // True when device is mobile or iOS — signals that animation budget is capped
