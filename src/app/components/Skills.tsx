...
// ── Typing Quote ──────────────────────────────────────────────────────────
const QUOTE_PLAIN = `"I start with human research — not tools. AI enhances my findings, but never replaces the thinking. Every design decision is backed by `;
const QUOTE_RED   = `data, user feedback, and real-world testing`;
const QUOTE_END   = `."`;
...
function SkillRing({ label, value, sub, idx }: { label: string; value: number; sub: string; idx: number }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay: idx * 0.18, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
...
const STEPS = [
  {
    ...
    desc: "Start with people, not pixels. Understand the problem before designing solutions.",
    bullets: ["User interviews & direct observation", "Reading research papers & case studies", "Documenting pain points & patterns", "Identifying what users actually need"],
  },
  {
    code: "02 PROC", status: "COMPLETE" as const, title: "AI Enhancement",
    desc: "AI validates findings and surfaces patterns — it doesn't replace research.",
    bullets: ["Enhance & validate research insights", "UX architecture recommendations", "Competitive analysis support", "Synthesize large datasets into insights"],
  },
  {
    code: "03 SYNC", status: "COMPLETE" as const, title: "Stakeholder Sync",
    desc: "Align with clients and devs before opening Figma. No surprises later.",
    bullets: ["Present research findings to stakeholders", "Collaborate with dev team on feasibility", "Identify what needs to change", "Agree on scope, priorities, and timeline"],
  },
  {
    code: "04 DSGN", status: "ACTIVE" as const, title: "Figma Design",
    desc: "Design backed by research, not guesswork. Every decision has a reason.",
    bullets: ["Wireframes → hi-fi UI", "Component-based design system", "Interactive prototype flows", "Design decisions tied to research data"],
  },
  {
    code: "05 PLSH", status: "QUEUED" as const, title: "Kiro Polish",
    desc: "Import design, optimize interactions, and refine before handoff.",
    bullets: ["Import Figma to code", "Refine animations & micro-interactions", "Performance optimization", "Accessibility review & testing"],
  },
  {
    code: "06 HNDOFF", status: "QUEUED" as const, title: "Dev Handoff",
    desc: "Clean specs, documented components, and direct communication.",
    bullets: ["Export assets & design specs", "Document component library", "Final QA review", "Hand off to development with support"],
  },
];
