...
export function Contact() {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!formRef.current || sendCooldown) return;
    setSending(true);
    try {
      ...
      setTimeout(() => setSendCooldown(false), 30000);
    } catch {
  ...
  return (
    <section id="contact" className="relative py-24 bg-[#050505] overflow-hidden">
      ...
      <div
        className="absolute top-1/2 left-1/4 w-96 h-96 rounded-full pointer-events-none hidden md:block"
            ...
            <form ref={formRef} onSubmit={submit} className="space-y-4">
              ...
              >
                ...
                <span className="relative">
                  {sending ? "TRANSMITTING..." : sendCooldown ? "MSG_SENT ✓" : "SEND_MSG"}
                </span>
