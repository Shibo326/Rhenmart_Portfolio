...
export function LoadingScreen({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const lineTimer = setInterval(() => setShown((s) => Math.min(s + 1, LINES.length)), 420);
    const progTimer = setInterval(() => setProgress((p) => Math.min(p + 4, 100)), 80);

    // Max wait: 2.5s hide + 400ms fade = 2.9s total
