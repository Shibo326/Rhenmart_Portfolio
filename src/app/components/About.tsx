...
const PARAGRAPHS = [
  "Self-taught UI/UX Designer and Bachelor of Science in Information Technology (BSIT) student from the Philippines. 5 years of learning by doing — not from courses, but from competitions, workshops, and real projects. I design interfaces that work because I start with research, not assumptions.",
  "My workflow: human research first (interviews, observation, papers), then AI to enhance findings and validate decisions. Figma for prototyping, close collaboration with devs to make sure designs actually ship. No shortcuts — just intentional work backed by data.",
  "5 competition wins taught me to work fast under pressure. 8 projects taught me to listen to users. Teaching Figma workshops taught me to explain design decisions clearly. Every project is a chance to get better.",
];
          ...
          >
            ...
            <div ref={tiltRef} className="relative animate-float tilt-3d">
              ...
              <div className="relative aspect-[3/4] overflow-hidden border border-[rgba(255,0,0,0.40)] rounded-t-[10rem] rounded-b-[2rem] group">
                ...
                {/* Scan beam — uses transform for GPU compositing, no layout reflow */}
                <div className="absolute inset-0 overflow-hidden pointer-events-none">
                  <div
                    className="absolute left-0 right-0 h-16 bg-gradient-to-b from-transparent via-[rgba(255,0,0,0.35)] to-transparent"
            ...
            {/* Floating skill badges — only on lg+ to avoid tablet overflow */}
            <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 lg:translate-x-12 space-y-3 hidden lg:block">
              {FLOATING.map((f, i) => (
                <motion.div
                  key={f}
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                className={`group relative border border-[rgba(255,255,255,0.08)] hover:border-[#FF0000] text-[rgba(255,255,255,0.55)] hover:text-white font-mono text-xs tracking-widest px-5 py-3 flex items-center gap-2 transition-all hover:scale-105 cursor-pointer overflow-hidden ${cvLoading || cvCooldown ? "opacity-60 pointer-events-none" : ""}`}
              >
                ...
                <span className="relative">{cvLoading ? "GENERATING..." : cvCooldown ? "DOWNLOADED ✓" : "DL_RESUME.PDF"}</span>
              </button>
            </div>
          </motion.div>
        </div>
