...
function Typer() {
  ...
  return (
    <span className="text-gradient-red font-bold" aria-live="polite" aria-atomic="true">
      {text}
      <span className="inline-block w-[3px] h-[0.9em] bg-[#FF0000] align-middle ml-1 animate-blink" aria-hidden="true" />
    </span>
  );
}
...
export function Hero() {
  ...
  const { enable3DScene, isMobile, isSafari, enableInfiniteLoops } = useAnimationConfig();
  const [cvLoading, setCvLoading] = useState(false);
  const [cvCooldown, setCvCooldown] = useState(false);

  const handleDownloadCV = async () => {
    if (cvLoading || cvCooldown) return;
    setCvLoading(true);
    try {
      await generateResume();
      setCvCooldown(true);
      setTimeout(() => setCvCooldown(false), 30000);
    } finally {
      setCvLoading(false);
    }
  };
  ...
  return (
    <section id="home" className="relative min-h-screen pt-24 pb-16 bg-[#050505]">
      ...
      {/* Particles — desktop only. Do NOT render on mobile: eliminates 20 DOM nodes
          and their animation computation entirely, preventing GPU overload on mobile. */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative" style={{ overflow: "visible" }}>
            ...
            <motion.div className="relative mx-auto w-[75%] lg:w-[68%] max-w-[260px] lg:max-w-none aspect-[3/4] z-20 mt-6">
              ...
              <div className="relative w-full h-full overflow-hidden border border-[rgba(255,0,0,0.50)] group cut-corner-lg depth-shadow">
                ...
                <div className="absolute inset-x-0 top-0 h-8 pointer-events-none"
                  style={{
                    background: "linear-gradient(to bottom, rgba(255,0,0,0.45), transparent)",
                    animation: "portraitScan 3s cubic-bezier(0.4,0,0.6,1) infinite",
                  }}
                />
          ...
          <div className="order-last lg:order-first lg:col-span-7 flex flex-col items-center lg:items-start space-y-7 relative z-10 text-center lg:text-left" style={{ overflow: "visible" }}>
            ...
            >
              ...
              <span className="relative flex h-2 w-2 shrink-0">
                <span className={`absolute inline-flex h-full w-full rounded-full bg-[#FF0000] opacity-60 ${enableInfiniteLoops ? 'animate-ping' : ''}`} />
            ...
            >
              Self-taught designer from the Philippines. I turn{" "}
              <span className="text-white">user research</span> into{" "}
              <span className="text-[#FF0000]">intentional interfaces</span> — no shortcuts, just real work. 5 competition wins, countless prototypes, and a dev-friendly mindset.
            </motion.p>
            ...
            >
              ...
              >
                ...
                <span className="relative flex h-2 w-2">
                  <span className={`absolute inline-flex h-full w-full rounded-full bg-[#FF0000] opacity-75 group-hover:bg-white ${enableInfiniteLoops ? 'animate-ping' : ''}`} />
        ...
        <div className="flex mt-12 flex-col items-center gap-2">
          <span className={`font-mono text-[10px] tracking-widest text-[rgba(255,255,255,0.55)] ${enableInfiniteLoops ? 'animate-blink' : ''}`}>
            [ SCROLL_DOWN ]
          </span>
          <span className="relative w-px h-12 bg-gradient-to-b from-[#FF0000] to-transparent overflow-hidden">
            <span className={`absolute inset-x-0 top-0 h-3 bg-[#FF0000] hidden sm:block ${enableInfiniteLoops ? 'animate-beam' : ''}`} />
          </span>
        </div>
      </div>
    </section>
