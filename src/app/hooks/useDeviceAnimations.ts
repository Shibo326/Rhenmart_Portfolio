...
// On mobile, return a static MotionValue(0) — no spring computation on every
// scroll frame. On desktop, return the spring-backed scaleX as before.
export function useScrollProgress() {
  const isMobile = useIsMobile();
  const { scrollYProgress } = useScroll();
  const spring = useSpring(scrollYProgress, { stiffness: 100, damping: 30 });
  const staticValue = useMotionValue(0);
  // Return static value on mobile to eliminate per-frame spring computation
  return isMobile ? staticValue : spring;
}
